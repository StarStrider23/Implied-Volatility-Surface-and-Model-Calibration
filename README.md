...
To focus the analysis on the most informative region of the implied volatility surface, the dataset was filtered to retain only options within the near ATM region (0.8 $\le$ Forward Moneyness $\le$ 1.2). Implied volatilities were then extracted by numerically inverting the Black–Scholes pricing formula using an implementation of Brent's root-finding algorithm. Together with interpolation, this procedure was repeated for every option in the filtered dataset to construct the market implied volatility smiles/skews and surfaces.
...
The parameters of both models were estimated by minimising their respective objective functions for each of the three selected market regimes. In addition, the Heston model was calibrated using reduced samples of the same market data in order to investigate the effect of sample size on calibration performance and parameter stability. The smaller samples were determined by choosing a set of common stirkes for each regime. 

Finally, the calibrated models were evaluated by comparing their implied volatility surfaces and volatility smiles against the extracted market implied volatilities. Model performance was assessed both visually and quantitatively through the RMSE, mean absolute error (MAE) and mean relative error (MRE) between the market and model-implied volatilities ($(\sigma_{market}-\sigma_{model})/\sigma_{market}$). Additionally, raw and interpolated error heatmaps are presented in order to aid to asses model performance visually.
...
The results highlight clear differences in both model performance and calibration stability across the Heston and SVI frameworks. Overall, the SVI model consistently provides a significantly better fit to the implied volatility surface and smiles, achieving a near perfect replication across all regimes with very low RMSE values. In contrast, the Heston model reproduces the general shape of the volatility surface but exhibits systematic deviations, escpecially for shorter maturities and further away from the ATM region.

A key observation from the Heston calibration is the sensitivity of certain parameters to optimisation constraints. In particular, the upper bound on the mean reversion speed  $\kappa$ was varied between 15, 30, 50 and 100. When allowed to reach higher values, $\kappa$ frequently drifted within a wide range (approximately 30–99) which may suggest that it is not strongly identified by the calibration procedure and different $\kappa$ values give very similar calibration results. The other parameters such as $\theta$, $\nu_0$ and $\xi$ remained considerably more stable across all regimes. Finally, the correlation parameter $\rho$ also exhibited variability in some cases, ranging from -0.58 to -0.99, but retained its expected strongly negative sign in all calibrations which agrees with the empirical "leverage effect" for stocks and equities, i.e. the observation that the stock price and its volatility are inversely proportional.

In ordrer to understand the differences between the models performance, it's worth discussing the difference between the calibration processes. The Heston model is calibrated globally, fitting five parameters to tens and potentially hundreds of option prices simultaneously, which naturally introduces certain limitations in its flexibility. By contrast, the SVI parametrisation is performed separately for each maturity, allowing it to adapt more closely to the real shape of each volatility smile. This difference in dimensionality and calibration structure is a key reason for SVI’s superior empirical performance. However, it also reflects that the two models are designed for different purposes: Heston is a dynamic stochastic volatility model whereas SVI is a static surface parametrisation.

This interpretation is further supported by the experiments using reduced Heston samples, where calibration quality improved noticeably. The reduction in sample size combined with increased optimisation iterations gave lower error levels and better convergence, confirming that part of the full sample results are also related to numerical and optimisation constraints and not necessarily model specification alone.

From a numerical perspective, the calibration procedure also plays an important role. The Heston model was calibrated using a differential evolution algorithm (scipy.optimise.differential_evolution), but due to computational constraints the population size and number of iterations were reduced in the full sample setting (popsize = 10, iterations = 50). Even with a reduced number of popsize and iterations, calibration time was estimated to be around 30-90 minutes depending on sample size (970 - 2473 options in a full sample). Despite these limitations, the resulting relative errors remained moderate, typically in the range of approximately 4–7%. In contrast, smaller samples consisted only of 20-28 options which allowed for significantly more extensive optimisation (popsize = 15, iterations = 200) and 100% convergence rate. In addition, the smaller samples included the strikes just below the ATM region which, if considering the full samples, produced smaller errors and therefore better fit. This helped to reduce the errors substantially. The SVI calibration, on the other hand, performed using standard nonlinear optimisation (scipy.optimise.minimise), which was both faster and more accurate, further reinforcing its efficiency advantage. Using the differential evolution algorithm may provide even better results.

An additional methodological aspect concerns the representation of the error heatmaps. The reader may have noticed that the errors were plotted in strike space rather than forward moneyness (like for implied volatility smiles and surfaces). While forward moneyness is often more economically interpretable and commonly used, its use would eliminate the common grid structure across option maturities since multiple strikes map into different forward moneyness values across dates. This would result in a non-uniform grid, making heatmap construction inconsistent. Using strikes preserves a shared cross sectional structure across maturities and allows for more consistent visual comparison.

## Conclusion

This project analysed S&P 500 option data from 2010–2023 with the aim of extracting implied volatility surfaces and assessing the performance of two widely used models: the Heston stochastic volatility model and the SVI parametrisation. Across different market regimes (low, moderate and high volatiltiy regimes), both models were evaluated in terms of their ability to reproduce observed volatility smiles and surfaces.

The results show that the SVI model provides a consistently superior fit to market data with very low errors and closely replicating the shape of the implied volatility surface across all maturities and volatility regimes. In contrast, the Heston model captures the general structure of the surface, but exhibits systematic calibration errors, especially for short maturities and away from the ATM region. In addition, it is sensitive to optimisation constraints and sample selection.

Despite its weaker empirical fit, the Heston model remains valuable due to its dynamic structure and economic interpretability while SVI excels as a highly accurate static surface representation. 
